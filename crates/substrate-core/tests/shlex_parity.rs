//! Byte-for-byte parity with CPython's `shlex` at the SSH boundary.
//!
...
/// (input, what CPython's shlex.quote returns)
const CASES: &[(&str, &str)] = &[
    ("", "''"),
    ("simple", "simple"),
    ...
    ("TESTTOKEN123abc", "TESTTOKEN123abc"),
    ("/var/lib/k0s/manifests", "/var/lib/k0s/manifests"),
    ("unicode-é", "'unicode-é'"),
    ("trailing ", "'trailing '"),
    (" leading", "' leading'"),
    ("'", "''\"'\"''"),
...
fn shell_join_matches_cpython_shlex() {
    ...
    assert_eq!(
        shell_join(&argv),
        "'' simple 'with space' 'it'\"'\"'s' 'a'\"'\"'b'\"'\"'c' 'dq\"uote' 'new\nline' 'tab\there' 'semi;colon' 'pipe|x' 'amp&x' 'dollar$X' 'back`tick`' 'star*' 'q?' 'br[a]' 'brace{a}' 'paren(a)' 'lt<gt>' 'tilde~' 'hash#' 'bang!' 'back\\slash' safe_@%+=:,./- 192.168.2.203 s1-vm1 --join-token TESTTOKEN123abc /var/lib/k0s/manifests 'unicode-é' 'trailing ' ' leading' ''\"'\"'' ''\"'\"''\"'\"'' '\\'\"'\"''"
    );
}
